...
md"# Vectorisation on A64FX in Julia vs other compilers"
...
md"""
By using metaprogramming we can automatically generate all the functions to call the routines in the shared libraries we have compiled.

Metaprogramming is a powerful tool to automatically generate code that would be otherwise tedious to type. However the following snippet seems a bit unreadable, and it probably is, so use metaprogramming with care!
"""

# ╔═╡ 9fa359b2-2b31-431b-93e3-77f84b4a9098
for sym in ("simple", "recip", "sqrt", "exp", "sin", "pow"), compiler in ("llvm", "fujitsu"), mode in ("", "fast")
...
md"""
Here Julia is in the same ballpark as the other compilers (e.g. Fujitsu, vanilla LLVM). We can have a look at the LLVM IR generated by Julia and the native code.
"""
...
md"""
Here Julia is much slower than the Fujitsu compiler when using the (unsafe) flags `-Kilfunc` and `-Kfp_relaxed`, but on-par with it when these advanced optimisations are disabled. Overall, Julia is limited by the code that LLVM can compile to, but it is consistently on-par with it.
